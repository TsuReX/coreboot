...
int32_t ht_setup(FSPM_UPD *mupd) {
		...
		if (error != 0) {
				...
				ht_config.ProcessorHyperThreadingDisable = 1;
		} else {
				printk(BIOS_ERR, "Hyper Threading configuration:\n");
				printk(BIOS_ERR, "ProcessorHyperThreadingDisable: 0x%01X\n", ht_config.ProcessorHyperThreadingDisable);
		}

		mupd->FspmConfig.ProcessorHyperThreadingDisable = ht_config.ProcessorHyperThreadingDisable;
