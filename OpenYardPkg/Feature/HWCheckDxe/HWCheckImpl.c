/** @file
  HII Config Access protocol implementation of HWCheckDxe driver.
...
EFI_GUID   mHWCheckVarGuid = HW_CHECK_VAR_GUID;
CHAR16     mHWCheckVarName[] = HW_CHECK_VAR_NAME;

HW_CHECK_CONFIG_PRIVATE_DATA mHWCheckConfigPrivateDataTemplate = {
  HW_CHECK_CONFIG_PRIVATE_DATA_SIGNATURE,
  {
    0,
    0,
    0
...
HII_VENDOR_DEVICE_PATH       mHWCheckHiiVendorDevicePath = {
  {
    {
  ...
  )
{
  ...
  EFI_GUID gHWCheckFormSetGuid = HW_CHECK_FORM_SET_GUID;
  ...
  BufferSize = sizeof(HW_CHECK_CONFIGURATION);
  ...
  )
{
  ...
  case EFI_BROWSER_ACTION_CHANGING:
  {
    if (QuestionId == 0x1250) {
      Status = HWCheckSaveHardwareInfo();
      if (!EFI_ERROR(Status))
      {
        PopupMessage = HiiGetString(ConfigPrivate->HiiHandle, STRING_TOKEN(STR_CURRENT_SAVED), NULL);
        Attribute = EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE;
      }
      else
      {
        PopupMessage = HiiGetString(ConfigPrivate->HiiHandle, STRING_TOKEN(STR_CURRENT_SAVE_ERROR), NULL);
        Attribute = EFI_LIGHTGRAY | EFI_BACKGROUND_RED;
      }
