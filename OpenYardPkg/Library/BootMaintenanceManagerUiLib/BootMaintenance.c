  ...
  )
{
  ...
  *Progress = Configuration;
  ...
  if (CompareMem (&NewBmmData->EnableShellBoot, &OldBmmData->EnableShellBoot, sizeof (NewBmmData->EnableShellBoot)) != 0) {
    Status = gRT->SetVariable (
                    L"ShellBoot",
                    &gEfiGlobalVariableGuid,
                    EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof (BOOLEAN),
                    &(NewBmmData->EnableShellBoot)
                    );
    if (EFI_ERROR (Status)) {
      Offset = OFFSET_OF (BMM_FAKE_NV_DATA, EnableShellBoot);
      goto Exit;
    }

    Private->BmmOldFakeNVData.EnableShellBoot = NewBmmData->EnableShellBoot;
  }
  
  if (CompareMem (&NewBmmData->EnableNetworkBoot, &OldBmmData->EnableNetworkBoot, sizeof (NewBmmData->EnableNetworkBoot)) != 0) {
    Status = gRT->SetVariable (
                    L"NetworkBoot",
                    &gEfiGlobalVariableGuid,
                    EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof (BOOLEAN),
                    &(NewBmmData->EnableNetworkBoot)
                    );
    if (EFI_ERROR (Status)) {
      Offset = OFFSET_OF (BMM_FAKE_NV_DATA, EnableNetworkBoot);
      goto Exit;
    }

    Private->BmmOldFakeNVData.EnableNetworkBoot = NewBmmData->EnableNetworkBoot;
  }
  ...
  )
{
  ...
  switch (Private->BmmPreviousPageId) {
    ...
    case FORM_SHELL_BOOT_ID:
      CurrentFakeNVMap->EnableShellBoot = Private->BmmOldFakeNVData.EnableShellBoot;
      break;
	  
    case FORM_NETWORK_BOOT_ID:
      CurrentFakeNVMap->EnableNetworkBoot = Private->BmmOldFakeNVData.EnableNetworkBoot;
      break;
  ...
  )
{
  ...
  // Initialize EnableShellBoot from NVRAM variable
  VarSize = sizeof (CallbackData->BmmFakeNvData.EnableShellBoot);
  Status = gRT->GetVariable (
                  L"ShellBoot",
                  &gEfiGlobalVariableGuid,
                  NULL,
                  &VarSize,
                  &CallbackData->BmmFakeNvData.EnableShellBoot
                  );
  if (EFI_ERROR (Status)) {
    // If variable doesn't exist, use PCD default
    CallbackData->BmmFakeNvData.EnableShellBoot = PcdGetBool (PcdEnableShellBootOption);
    DEBUG ((DEBUG_INFO, "ShellBoot using PCD default: %d\n", CallbackData->BmmFakeNvData.EnableShellBoot));
  } else {
    DEBUG ((DEBUG_INFO, "ShellBoot loaded from NVRAM: %d\n", CallbackData->BmmFakeNvData.EnableShellBoot));
  }
  
  // Initialize EnableNetworkBoot from NVRAM variable  
  VarSize = sizeof (CallbackData->BmmFakeNvData.EnableNetworkBoot);
  Status = gRT->GetVariable (
                  L"NetworkBoot",
                  &gEfiGlobalVariableGuid,
                  NULL,
                  &VarSize,
                  &CallbackData->BmmFakeNvData.EnableNetworkBoot
                  );
  if (EFI_ERROR (Status)) {
    // If variable doesn't exist, use PCD default
    CallbackData->BmmFakeNvData.EnableNetworkBoot = PcdGetBool (PcdEnableNetworkBootOption);
    DEBUG ((DEBUG_INFO, "NetworkBoot using PCD default: %d\n", CallbackData->BmmFakeNvData.EnableNetworkBoot));
  } else {
    DEBUG ((DEBUG_INFO, "NetworkBoot loaded from NVRAM: %d\n", CallbackData->BmmFakeNvData.EnableNetworkBoot));
  }
