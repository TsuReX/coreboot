...
//UINT16    mKeyInput;
EFI_GUID  mSecurityGuid = SECURITY_FORMSET_GUID;
//
...
SECURITY_CALLBACK_DATA  gSecurityPrivate = {
  SECURITY_CALLBACK_DATA_SIGNATURE,
  NULL,
  NULL,
  {
...
HII_VENDOR_DEVICE_PATH  mSecurityHiiVendorDevicePath = {
  {
    {
  ...
  //
  // {1DDDBE15-481D-4d2b-8277-B191EAF66536}
  //
  { 0x1dddbe15, 0x481d, 0x4d2b, { 0x82, 0x77, 0xb1, 0x91, 0xea, 0xf6, 0x65, 0x37 }
  }
},
{
...
**/
CHAR16*
SecurityExtractDevicePathFromHiiHandle(
...
//  UINTN               AddItemCount;
  ...
  HiiHandle = gSecurityPrivate.HiiHandle;
  ...
  for (Index = 0; HiiHandles[Index] != NULL; Index++) {
    ...
    while (TempSize < BufferSize) {
      ...
      while (ClassGuidNum-- > 0) {
        if (CompareGuid(&gEfiHiiPlatformSecurityFormsetGuid, ClassGuid) == 0) {
          ClassGuid++;
          continue;
        }
        ...
        //
        if (NextShowFormId == FORM_SECURITY_INFO_ID) {
          DevicePathStr = SecurityExtractDevicePathFromHiiHandle(HiiHandles[Index]);
          DevicePathId = 0;
...
{
  if ((Securityuration == NULL) || (Progress == NULL)) {
    return EFI_INVALID_PARAMETER;
  }

  *Progress = Securityuration;
  ...
  )
{
  ...
  //
  // Install Device Path Protocol and Security Access protocol to driver handle
  //
  gSecurityPrivate.DriverHandle = NULL;
  Status                           = gBS->InstallMultipleProtocolInterfaces (
                                            &gSecurityPrivate.DriverHandle,
                                            &gEfiDevicePathProtocolGuid,
                                            &mSecurityHiiVendorDevicePath,
                                            &gEfiHiiConfigAccessProtocolGuid,
                                            &gSecurityPrivate.SecurityAccess,
                                            NULL
                                            );
  ...
  //
  gSecurityPrivate.HiiHandle = HiiAddPackages (
                                    &mSecurityGuid,
                                    gSecurityPrivate.DriverHandle,
                                    SecurityVfrBin,
                                    SecurityUiLibStrings,
                                    NULL
                                    );
  ASSERT (gSecurityPrivate.HiiHandle != NULL);

  CreateSecurityForm(FORM_SECURITY_INFO_ID);
  ...
  )
{
  ...
  HiiRemovePackages (gSecurityPrivate.HiiHandle);
