  ...
  )
{
  ...
  // Для ShellBoot - устанавливаем только если переменная не существует
  VarSize = 0;
  Status = gRT->GetVariable (
                  L"ShellBoot",
                  &gEfiGlobalVariableGuid,
  ...
  if (Status == EFI_NOT_FOUND) {
    // Переменной нет - устанавливаем значение по умолчанию из PCD
    EnableShellBoot = (BOOLEAN) PcdGetBool (PcdEnableShellBootOption);
    BdsDxeSetVariableAndReportStatusCodeOnError (
      (CHAR16*)L"ShellBoot",
      &gEfiGlobalVariableGuid,
      EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
    ...
    DEBUG ((DEBUG_INFO, "Initialized ShellBoot to %d\n", EnableShellBoot));
  } else {
    DEBUG ((DEBUG_INFO, "ShellBoot variable already exists, keeping current value\n"));
  }
  
  // Для NetworkBoot - устанавливаем только если переменная не существует
  VarSize = 0;
  Status = gRT->GetVariable (
                  L"NetworkBoot",
                  &gEfiGlobalVariableGuid,
  ...
  if (Status == EFI_NOT_FOUND) {
    // Переменной нет - устанавливаем значение по умолчанию из PCD
    EnableNetworkBoot = (BOOLEAN) PcdGetBool (PcdEnableNetworkBootOption);
    BdsDxeSetVariableAndReportStatusCodeOnError (
      (CHAR16*)L"NetworkBoot",
      &gEfiGlobalVariableGuid,
      EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
      sizeof (BOOLEAN),
      &EnableNetworkBoot
      );
    DEBUG ((DEBUG_INFO, "Initialized NetworkBoot to %d\n", EnableNetworkBoot));
  } else {
    DEBUG ((DEBUG_INFO, "NetworkBoot variable already exists, keeping current value\n"));
  }
