...
// OYSetup PCH USB
OY_SETUP_CONFIGURATION gOYSetupConfig;
  ...
  )
{
  ...
  // OYSetup USB read only
  
  if ( (gOYSetupConfig.PchUsbReadOnly == 0x01 ) ) // || (Media->ReadOnly == TRUE) )
  {
	Status = EFI_SUCCESS;  // not to trigger fat driver error
    goto ON_EXIT;
  }
  ...
  )
{
  ...
  if (gOYSetupConfig.PchUsbReadOnly == 0x01)
  {
	  Media->ReadOnly         = TRUE;
  }
  ...
  )
{
  ...
  // OYSetup check if no mass storage enabled
  
  UINTN                       VarSize;
  
  VarSize = sizeof(gOYSetupConfig);
  Status = gRT->GetVariable(
    OY_SETUP_VARIABLE_NAME,
    &gEfiOYSetupVariableGuid,
    NULL,
    &VarSize,
    &gOYSetupConfig
  );
  if (!EFI_ERROR(Status))
  {
    if (gOYSetupConfig.PchUsbFilter == 0x01)
    {
